...
// replaceAll function code from http://stackoverflow.com/questions/7724011/in-c-whats-the-fastest-way-to-replace-all-occurrences-of-a-substring-within
std::string
...
std::string DownloadPage( const std::string& _Class, int _StartWeek, int _EndWeek )
{
	...
	ss << "output\\output_" << _Class << "_" << _StartWeek << "-" << _EndWeek << ".html";
...
void DeleteDownloadedFile( const std::string& _HTMLFile )
{
	std::cout << "> Deleting intermediate file: " << _HTMLFile << std::endl;
	std::stringstream ss;
	ss << "Del /F /Q /S \"" << _HTMLFile << "\"";
	system(ss.str().c_str());
}
...
SICSCAPI bool ProcessSchedule( std::string& _ClassCode, int _StartYear, int _StartWeek, int _EndWeek )
{
	...
	classcode = replaceAll(_ClassCode, " ", "%20");	// Replace every space that is present with %20 so that the URL will work.

	std::string htmlfile = DownloadPage(classcode, startweek, endweek);

	std::string ical;
	if (ConvertHTMLToiCalString(htmlfile, ical, year))
	{
		std::stringstream iCalFile;
		std::string filename = classcode;
		filename = replaceAll(filename, "%20", "_");	// Convert our previously-added spaces to underscores.
