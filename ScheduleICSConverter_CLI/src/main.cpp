...
int main( int _Argc, char* _Argv[] )
{
	...
	else if (strcmp(_Argv[1], "1") == 0)	// Command line mode.
	{
		...
		int year = atoi(_Argv[3]);	// This requires classcodes passed in from the command line to be formatted with %20 in place of spaces.
	...
	{
		std::cout << "Auto mode temporarily disabled while the software is changed to support non-IGAD courses!";

// 		int year = atoi(_Argv[3]);
// 		int startweek = atoi(_Argv[4]);
// 		int endweek = atoi(_Argv[5]);
// 
// 		int maxClass = 20;	// Used to 15, changed to accommodate non-IGAD courses.
// 		int maxYear = 6;
